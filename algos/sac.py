...
class SAC(RLAlgorithm, Serializable):
    ...
    def __init__(
        ...
        self._g = g
        self._policy = policy
        self._sub_level_policies_paths =sub_level_policies_paths
        ...
        '''with self._sess.as_default():
            with tf.variable_scope("fwrd2", reuse=False):
                fwrd = joblib.load("hc/fwd/itr_2000.pkl")
            with tf.variable_scope("longx2", reuse=False):
                bwrd = joblib.load("hc/jp-longx/itr_2000.pkl")
            with tf.variable_scope("longz2", reuse=False):
                uwrd = joblib.load("hc/jp-longz/itr_2000.pkl")
            with tf.variable_scope("dwrd", reuse=False):
                dwrd = joblib.load("ant4-5000/dwrd/itr_5000.pkl")
        ...
        self._sub_level_policies =sub_level_policies'''		

    @overrides
    def train(self):
        """Initiate training of the SAC instance."""

        self._train(self._env, self._policy, self._initial_exploration_policy, self._sub_level_policies_paths,self._pool, self._g)
    ...
    def _init_placeholders(self):
        ...
        self.sub_level_actions = tf.placeholder(
            tf.float32,
            shape=(None,None,self._Da),
            name='sub_level_action',
        )
        self.sub_level_entropies = tf.placeholder(
            tf.float32,
            shape=(None,None,1),
            name='sub_level_entropies',
        )
    ...
    def _init_actor_update(self):
        ...
        """
        actions,sm_scores,squash_correction = self._policy.actions_for(observations=self._observations_ph,sub_level_actions=self.sub_level_actions,with_log_pis=True)
        log_pi = tf.log(tf.reduce_sum(tf.multiply(self.sub_level_entropies,tf.expand_dims(sm_scores,2)),1))
        #log_pi = tf.reduce_sum(tf.multiply(self.sub_level_entropies,tf.expand_dims(sm_scores,2)),1)
        log_pi-=squash_correction
        #actions = tf.reduce_sum(tf.multiply(self.sub_level_actions,tf.expand_dims(sm_scores,2)),1)
    ...
    def _do_training(self, iteration, batch):
        ...
        #action_=self._sess.run(self.actions_, feed_dict)
        #print(np.shape(action_))
        #input()
    ...
    def _get_feed_dict(self, iteration, batch):
        ...
        '''sub_level_actions=[]
        obs_low=batch['observations']
        for i in range(0,len(self._sub_level_policies)):
            sub_level_actions.append(self._sub_level_policies[i].get_actions(obs_low[:,:-2]))
        sub_level_actions=np.stack(sub_level_actions,axis=0)
        sub_level_actions=np.transpose(sub_level_actions,(1,0,2))

        sub_level_entropies=[]
        for i in range(0,len(self._sub_level_policies)):
            sub_level_entropies.append(np.exp(self._sub_level_policies[i].log_pis_for(obs_low[:,:-1])))#dnt forget to remove exp
        sub_level_entropies= np.stack(sub_level_entropies,axis=0)
        sub_level_entropies=np.transpose(sub_level_entropies,(2,0,1))'''
        #print(sub_level_actions)
        #input()
        feed_dict = {
            self._observations_ph: batch['observations'],
            self.sub_level_actions:batch['sub_level_actions'],#sub_level_actions,# 
            self.sub_level_entropies:batch['sub_level_probs'],#sub_level_entropies,
